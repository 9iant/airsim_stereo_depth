...
void AirsimROSWrapper::rpy_thrust_cmd_cb(const mavros_msgs::AttitudeTarget& attitude_cmd_msg){
    ...
    // roll = atan2(2 * (cmd_quater.w() * cmd_quater.x() + cmd_quater.y() * cmd_quater.z()), 1 - 2 * (cmd_quater.x() * cmd_quater.x() + cmd_quater.y() * cmd_quater.y()));
    // pitch = asin(2 * (cmd_quater.w() * cmd_quater.y() - cmd_quater.z() * cmd_quater.x()));
    // yaw = atan2(2 * (cmd_quater.w() * cmd_quater.z() + cmd_quater.x() * cmd_quater.y()), 1 - 2 * (cmd_quater.y() * cmd_quater.y() + cmd_quater.z() * cmd_quater.z()));
    // static int a = 0;
    if(isENU_){
        // a++;
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_truth_state(const msr::airlib::Kinematics::State& truth_state) const
{
    ...
    if (isENU_)
    {
        ...
        // std::cout<<"----------------------------------------\n";
        // std::cout<<rota<<"\n";
        // std::cout<<"111Enu: "<<"w: "<<enuq.w()<<" x: "<<enuq.x()<<" y: "<<enuq.y()<<" z: "<<enuq.z()<<"\n";
        // std::cout<<"rot: \n"<<rota<<std::endl;
        odom_msg.pose.pose.orientation.w = enuq.w();
        odom_msg.pose.pose.orientation.x = enuq.x();
        odom_msg.pose.pose.orientation.y = enuq.y();
